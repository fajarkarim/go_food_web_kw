...
import { render, fireEvent, waitFor, screen, waitForElementToBeRemoved, act} from '@testing-library/react'
...
describe('SearchRestaurants', () => {
  ...
  describe('#render', () => {
    it('should render Search button and Input Search successfully', async () => {
  ...
  describe('#handleOnSearch', () => {
    it('should call fetchRestaurants with expected query when search input'
      ...
      const fetchRestaurantsSpy = jest.spyOn(RestaurantsService, 'fetchRestaurants');
    ...
    it(`should render 2 restaurant's cards when get response from Zomato API call`,async () => {
      render(<SearchRestaurants/>);
      ...
      const firstRestaurantCardComponent = screen.getByTestId(`restaurant-${firstRestaurantName}`);
      const secondRestaurantCardComponent = screen.getByTestId(`restaurant-${secondRestaurantName}`);
